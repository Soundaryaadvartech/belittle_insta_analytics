        ...
        if is_access_token_expired(BLT_ACCESS_TOKEN):
            try:
                ...
                try:
                    ...
                    load_dotenv()  # Reload the updated .env file
                    # Now use the new long-lived token to generate a new short-lived (BLT) access token
                    # Here we assume that generate_blt_access_token uses long-lived token to create the short-lived one
...
def engaged_audience_demographics(db: Session = Depends(get_db)):
    try:
        ...
        engaged_audience_age_url = f"{BASE_URL}{BLT_INSTAGRAM_ACCOUNT_ID}/insights?metric=engaged_audience_demographics&period=lifetime&timeframe=this_week&metric_type=total_value&breakdown=age&access_token={BLT_ACCESS_TOKEN}"
        engaged_audience_age_response = requests.get(engaged_audience_age_url, timeout=60)
        engaged_audience_gender_url = f"{BASE_URL}{BLT_INSTAGRAM_ACCOUNT_ID}/insights?metric=engaged_audience_demographics&period=lifetime&timeframe=this_week&metric_type=total_value&breakdown=gender&access_token={BLT_ACCESS_TOKEN}"
        engaged_audience_gender_response = requests.get(engaged_audience_gender_url, timeout=60)
        engaged_audience_city_url = f"{BASE_URL}{BLT_INSTAGRAM_ACCOUNT_ID}/insights?metric=engaged_audience_demographics&period=lifetime&timeframe=this_week&metric_type=total_value&breakdown=city&access_token={BLT_ACCESS_TOKEN}"
...
def fetch_all_posts(db: Session = Depends(get_db)):
    try:
        ...
        posts_url = f"{BASE_URL}{BLT_INSTAGRAM_ACCOUNT_ID}/media?fields=id,media_type,media_url,timestamp&access_token={BLT_ACCESS_TOKEN}"
        while posts_url:
        ...
        for post in all_posts:
            ...
            insights_url = f"{BASE_URL}{post_id}/insights?metric=reach&access_token={BLT_ACCESS_TOKEN}"
            insights_response = requests.get(insights_url,  timeout=60)
            ...
            # Fetch saves
            saves_url = f"{BASE_URL}{post_id}/insights?metric=saved&access_token={BLT_ACCESS_TOKEN}"
